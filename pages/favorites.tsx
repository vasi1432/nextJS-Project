...
export const getStaticProps = async () => {
  const res = await fetch("https://gateway.oslash.work/graphql", {
    ...
    body: JSON.stringify({
      query: `
        query GetAllFavorites($workspacePK: String!) {
      getAllFavorites(workspacePK: $workspacePK) {
        pk
        sk
        favoriteItem {
          ... on CollectionShortcut {
            _version
            collection {
              countSharedGroups
              countSharedMembers
...
const Shortcuts = ({ favorites }) => {

    
  return (
    <>
      <PageLabel shortcuts={favorites.data.getAllFavorites.length}></PageLabel>
      <div className="relative left-40 top-32 w-2/3 flex flex-col	gap-3">
        ...
        {favorites.data.getAllFavorites.map(
          (item: {
            [x: string]: any;
            uid: Key;
            shortlink:
            ...
            return (
              ...
              >
                ...
                ></img>
                <div>
                  <p>{item.favoriteItem.shortlink}</p>
                  <p className="text-gray-500	">
                    {item.favoriteItem.description}
                  </p>
